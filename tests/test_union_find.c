...
int main(void) {
    /* Create UF of size 5, verify find(i)==i for all i */
    ...
    /* Union(0,1): verify connected(0,1)==true, connected(0,2)==false */
    uf_union(uf, 0, 1);
    ...
    /* Test reset: after reset, verify find(i)==i again */
    uf_reset(uf);
    for (int i = 0; i < 5; i++) {
